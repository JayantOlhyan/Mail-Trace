      ...
      <div className="sticky top-0 flex h-screen items-center justify-center flex-col px-4 md:px-24" style={{ opacity: sceneOpacity }}>
        ...
        <div className="w-full max-w-5xl grid md:grid-cols-2 gap-8 items-stretch">
          ...
          <div className="flex flex-col justify-center gap-8">
            ...
            {/* Parsed & Normalized */}
            <div 
              className={`bg-slate-100 dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-lg p-6 transition-all duration-700 transform ${showParsed ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
            >
              <div className="text-xs font-mono text-slate-500 tracking-wider mb-4 border-b border-slate-200 dark:border-slate-800 pb-2">
                NORMALIZED DATA
              </div>
              <div className="space-y-2 font-mono text-sm">
                <div className="grid grid-cols-[100px_1fr] text-slate-700 dark:text-slate-300">
            ...
            {/* Analyzed Auth */}
            <div 
              className={`bg-slate-100 dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-lg p-6 transition-all duration-700 delay-150 transform ${showAnalyzed ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
            >
              <div className="text-xs font-mono text-slate-500 tracking-wider mb-4 border-b border-slate-200 dark:border-slate-800 pb-2">
                ...
                {headers.parsed.map((auth, idx) => (
                  <div key={idx} className="flex items-center justify-between bg-red-950/20 border border-red-900/30 p-2 rounded">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-slate-600 dark:text-slate-400 text-xs">{auth.detail}</span>
                    </div>

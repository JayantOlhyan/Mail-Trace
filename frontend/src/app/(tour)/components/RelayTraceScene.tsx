    ...
    <section id="scene-07" className="relative min-h-[200vh]" ref={ref}>
      <div className="sticky top-0 flex h-screen items-center justify-center md:justify-between px-4 md:px-24">
        ...
        <div className="hidden md:block w-1/3">
          ...
          <p className="text-xs font-mono text-slate-500 bg-slate-900/50 p-3 border border-slate-800 rounded">
              ...
              return (
                <div key={idx} className="flex flex-col items-center w-full max-w-sm">
                  ...
                  <button 
                    ...
                    className={`w-full bg-slate-900 border ${activeHop === idx ? 'border-blue-500' : 'border-slate-800'} ${isInteractable ? 'cursor-pointer hover:border-slate-600' : 'cursor-default'} rounded-lg p-4 flex items-center justify-between transition-all duration-500 transform ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4'}`}
                  >
                    <div className="flex items-center gap-4">
                      <div className="w-10 h-10 bg-slate-950 rounded flex items-center justify-center border border-slate-800 text-slate-400">
                        <Server className="w-5 h-5" />
                      </div>
                      <div className="text-left">
                        <div className="text-xs font-bold text-slate-500 tracking-wider mb-1">{hop.label}</div>
                        <div className="text-sm font-mono text-slate-200">{hop.detail}</div>
                      </div>
                    </div>
                  </button>
                  ...
                  {isInteractable && (
                    ...
                    >
                      <div className="bg-slate-950 border border-slate-800 rounded p-4 text-xs font-mono text-slate-400 space-y-2">
                        <div className="grid grid-cols-2">
                          <span>ASN:</span><span className="text-slate-300">{relay.infrastructure.asn}</span>
                        </div>
                        <div className="grid grid-cols-2">
                          <span>NETWORK:</span><span className="text-slate-300">{relay.infrastructure.network}</span>
                        </div>
                        <div className="grid grid-cols-2">
                          <span>GEOLOCATION:</span><span className="text-slate-300">{relay.infrastructure.geolocation}</span>
                        </div>
                        <div className="grid grid-cols-2">
                          <span>CONFIDENCE:</span><span className="text-blue-400">{relay.infrastructure.confidence}</span>
                        </div>
                        <div className="mt-4 pt-2 border-t border-slate-800 text-slate-500 text-[10px]">
